program main
    ...
    integer,parameter :: Tsteps=15000, N=500
    real, parameter :: dt=1e-3, m=1./N, mtot=m*N, omega=1., G=1., R=1., eps=0.01
    ...
    do j=1,3 !substracting the center of mass velocity to every N bodies
        vcm(j) = sum(vel(:,j))/N
        vel(:,j) = vel(:,j)-vcm(j)
    enddo
    ...
    open(unit=14, file="angularmomentum.bin", form="unformatted", access='stream', status="replace")
    ...
    write(12) vel(:,:)
    write(13) Ek, Ep
    write(14) angularmomentum(pos,vel)
    ...
    contains
        ...
        function angularmomentum(r,v) result(Lnorm)
            real,dimension(N,3),intent(in) :: r,v
            real,dimension(3) :: L
            real :: Lnorm
            integer :: i
            ...
            L = 0.
            do i=1,N
                L(1) = L(1) + r(i,2)*v(i,3)-r(i,3)*v(i,2)
                L(2) = L(2) + r(i,3)*v(i,1)-r(i,1)*v(i,3)
                L(3) = L(3) + r(i,1)*v(i,2)-r(i,2)*v(i,1)
            enddo

            Lnorm = sqrt(sum(L*L))*m
